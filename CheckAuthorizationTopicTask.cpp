...
CheckAuthorizationTopicTask::CheckAuthorizationTopicTask(const std::string& username, const std::string& topicName, std::shared_ptr<boost::asio::ip::tcp::socket> socket)
    : Task(socket), username(username), topicName(topicName) {}

void CheckAuthorizationTopicTask::run() {
    Globals& globals = Globals::getInstance();
    Crypto& crypto = Crypto::getInstance();

    Json::Value responseJson = globals.checkAuthorizationTopic(username, topicName);
